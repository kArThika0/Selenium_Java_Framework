...
public class SeleniumFramework extends BaseTest{
    ...
    public void submitOrder(HashMap<String,String> input) throws IOException, InterruptedException {
        ...
        @Test(dataProvider = "getData", dependsOnMethods = {"submitOrder"}, groups = {"positive"})
        public void verifyPlacedOrder(HashMap<String,String> input ) {
            //Verify placed order
            ProductCataloguePage productCataloguePage = login.loginIntoPage(input.get("email"), input.get("password"));
            ...
            OrdersPage ordersPage = checkoutpage.goToOrdersPage();
            WebElement orderConfirmMsg = ordersPage.verifyOrdersPage();
            Assert.assertEquals(orderConfirmMsg.getText(), "Your Orders");
            // Go to orders and verify the order placed
            boolean orderMatch = ordersPage.verifyOrdersByOrderNumber(orderNumber);
